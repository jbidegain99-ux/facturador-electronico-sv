...
let DteService = (() => {
    ...
    let _classThis;
    var DteService = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            DteService = _classThis = _classDescriptor.value;
        ...
        signerService;
        mhAuthService;
        logger = new common_1.Logger(DteService.name);
        constructor(prisma, signerService, mhAuthService) {
            this.prisma = prisma;
        ...
        isDemoMode(tenant) {
            ...
            if (process.env.DEMO_MODE === 'true')
                return true;
            if (tenant?.plan === 'DEMO' || tenant?.plan === 'TRIAL')
                return true;
            return false;
        }
        /**
         ...
         */
        generateDemoResponse(codigoGeneracion) {
        ...
        async createDte(tenantId, tipoDte, data) {
        ...
        async transmitDte(dteId, nit, password) {
            const dte = await this.prisma.dTE.findUnique({
                where: { id: dteId },
                include: { tenant: true },
            });
            ...
            try {
                // Get auth token
                const env = process.env.MH_API_ENV || 'test';
                const tokenInfo = await this.mhAuthService.getToken(nit, password, env);
                // Parse jsonOriginal from string
                const jsonOriginalParsed = typeof dte.jsonOriginal === 'string'
                    ? JSON.parse(dte.jsonOriginal)
                    : dte.jsonOriginal;
                const ambiente = (jsonOriginalParsed?.identificacion?.ambiente || '00');
                const request = {
                    ambiente,
                    idEnvio: Date.now(),
                    version: shared_1.DTE_VERSIONS[dte.tipoDte],
                    tipoDte: dte.tipoDte,
        ...
        async findByTenant(tenantId, page = 1, limit = 20, filters, sortBy, sortOrder) {
        ...
        async getNextCorrelativo(tenantId, tipoDte) {
        ...
        generateNumeroControl(tipoDte, correlativo) {
        ...
        async logDteAction(dteId, accion, data) {
        ...
        async getSummaryStats(tenantId) {
            ...
            const [dtesToday, dtesThisMonth, dtesLastMonth, totalThisMonth, totalLastMonth, rejectedCount,] = await Promise.all([
                // DTEs today
        ...
        async getStatsByDate(tenantId, startDate, endDate, groupBy = 'day') {
            // Handle case where user has no tenant assigned
            if (!tenantId) {
        ...
        async getStatsByType(tenantId, startDate, endDate) {
            // Handle case where user has no tenant assigned
            if (!tenantId) {
        ...
        async getStatsByStatus(tenantId) {
            // Handle case where user has no tenant assigned
            if (!tenantId) {
        ...
        async getTopClients(tenantId, limit = 10, startDate, endDate) {
            // Handle case where user has no tenant assigned
            if (!tenantId) {
            ...
            // Get client names
            const clientIds = stats.map((s) => s.clienteId).filter(Boolean);
        ...
        async getRecentDTEs(tenantId, limit = 5) {
            // Handle case where user has no tenant assigned
            if (!tenantId) {
