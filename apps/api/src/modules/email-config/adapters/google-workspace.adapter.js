...
exports.GoogleWorkspaceAdapter = void 0;
 ...
 */
class GoogleWorkspaceAdapter extends email_adapter_interface_1.BaseEmailAdapter {
    provider = email_types_1.EmailProvider.GOOGLE_WORKSPACE;
    logger = new common_1.Logger(GoogleWorkspaceAdapter.name);
    gmailUrl = 'https://gmail.googleapis.com/gmail/v1';
    ...
    async testConnection() {
        ...
        try {
            ...
            this.logger.warn(`Google Workspace connection test failed: ${errorMessage}`);
            return {
                success: false,
                responseTimeMs: Date.now() - startTime,
                errorMessage,
    ...
    buildMimeMessage(params) {
        ...
        const hasAttachments = 'attachments' in params && params.attachments?.length > 0;
        if (hasAttachments) {
    ...
    async sendEmail(params) {
        try {
            ...
            const encodedMessage = Buffer.from(mimeMessage)
                ...
                .replace(/\//g, '_')
                .replace(/=+$/, '');
            const response = await fetch(`${this.gmailUrl}/users/me/messages/send`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${accessToken}`,
            ...
            const data = await response.json();
            this.logger.debug(`Email sent successfully via Google Workspace. MessageId: ${data.id}`);
            return {
                success: true,
                messageId: data.id,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to send email via Google Workspace: ${errorMessage}`);
            return {
                success: false,
                errorMessage,
            };
    ...
    async sendEmailWithAttachment(params) {
        try {
            ...
            const data = await response.json();
            this.logger.debug(`Email with attachments sent successfully via Google Workspace. MessageId: ${data.id}`);
            return {
                success: true,
                messageId: data.id,
                providerResponse: data,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to send email with attachments via Google Workspace: ${errorMessage}`);
            return {
                success: false,
                errorMessage,
            };
...
exports.GoogleWorkspaceAdapter = GoogleWorkspaceAdapter;
