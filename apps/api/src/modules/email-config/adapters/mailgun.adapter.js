 ...
 */
class MailgunAdapter extends email_adapter_interface_1.BaseEmailAdapter {
    provider = email_types_1.EmailProvider.MAILGUN;
    logger = new common_1.Logger(MailgunAdapter.name);
    domain;
    baseUrl;
    constructor(config) {
        super(config);
    ...
    async sendEmail(params) {
        try {
            ...
            if (params.replyTo || this.config.replyToEmail) {
                formData.append('h:Reply-To', params.replyTo || this.config.replyToEmail);
            }
            ...
            this.logger.debug(`Email sent successfully via Mailgun. MessageId: ${data.id}`);
            return {
                success: true,
                messageId: data.id,
    ...
    async sendEmailWithAttachment(params) {
        try {
            ...
            const data = await response.json();
            this.logger.debug(`Email with attachments sent successfully via Mailgun. MessageId: ${data.id}`);
            return {
                success: true,
                messageId: data.id,
                providerResponse: data,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to send email with attachments via Mailgun: ${errorMessage}`);
            return {
                success: false,
                errorMessage,
            };
...
exports.MailgunAdapter = MailgunAdapter;
