 ...
 */
class SendGridAdapter extends email_adapter_interface_1.BaseEmailAdapter {
    provider = email_types_1.EmailProvider.SENDGRID;
    logger = new common_1.Logger(SendGridAdapter.name);
    ...
    async sendEmail(params) {
        try {
            ...
            this.logger.debug(`Email sent successfully via SendGrid. MessageId: ${messageId}`);
            return {
                success: true,
    ...
    async sendEmailWithAttachment(params) {
        try {
            ...
            const messageId = response.headers.get('x-message-id');
            this.logger.debug(`Email with attachments sent successfully via SendGrid. MessageId: ${messageId}`);
            return {
                success: true,
                messageId: messageId || undefined,
            };
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to send email with attachments via SendGrid: ${errorMessage}`);
            return {
                success: false,
                errorMessage,
            };
    ...
    async getQuotaStatus() {
        try {
            ...
            this.logger.warn(`Failed to get SendGrid quota: ${error instanceof Error ? error.message : 'Unknown'}`);
