...
exports.SmtpGenericAdapter = void 0;
 ...
 */
class SmtpGenericAdapter extends email_adapter_interface_1.BaseEmailAdapter {
    provider = email_types_1.EmailProvider.SMTP_GENERIC;
    logger = new common_1.Logger(SmtpGenericAdapter.name);
    transporter;
    constructor(config) {
        super(config);
    ...
    async sendEmail(params) {
        try {
            ...
            this.logger.debug(`Email sent successfully via SMTP. MessageId: ${info.messageId}`);
            return {
                success: true,
    ...
    async sendEmailWithAttachment(params) {
        try {
            ...
            const info = await this.transporter.sendMail(mailOptions);
            this.logger.debug(`Email with attachments sent successfully via SMTP. MessageId: ${info.messageId}`);
            return {
                success: true,
                messageId: info.messageId,
                providerResponse: info,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger.error(`Failed to send email with attachments via SMTP: ${errorMessage}`);
            return {
                success: false,
                errorMessage,
                errorCode: error.code,
...
exports.SmtpGenericAdapter = SmtpGenericAdapter;
