/**
 * Email template: Quote updated notification to client.
 * Sent when a quote is revised after client feedback.
 */
...
function buildLineItemsTable(
  lineItems: QuoteUpdatedData['lineItems'],
): string {
...
export function quoteUpdatedTemplate(
  data: QuoteUpdatedData,
  ...
  const body = `
    ...
    <p style="margin: 0 0 8px 0; font-size: 14px; color: ${COLORS.textSecondary}; font-family: Arial, sans-serif; line-height: 1.6;">
      Hemos actualizado la cotizaci&oacute;n <strong style="color: ${COLORS.textPrimary};">${escapeHtml(data.quoteNumber)}</strong>
      (versi&oacute;n ${data.version}) en base a los cambios solicitados.
    </p>
    <p style="margin: 0 0 24px 0; font-size: 14px; color: ${COLORS.textSecondary}; font-family: Arial, sans-serif; line-height: 1.6;">
      A continuaci&oacute;n el detalle actualizado:
    </p>
    ...
    ${emailButton('Ver Cotizacion Actualizada', data.approvalLink, COLORS.primary)}
  `;

  const html = emailLayout({
    title: 'Cotizacion Actualizada',
    subtitle: escapeHtml(data.quoteNumber),
    preheader: `Cotizacion ${escapeHtml(data.quoteNumber)} actualizada - ${formatMoney(data.total)}`,
    body,
  });
  ...
  const text = [
    `Cotizacion Actualizada - ${data.quoteNumber}`,
    '',
    `Estimado/a ${data.clienteNombre},`,
    '',
    `Hemos actualizado la cotizacion ${data.quoteNumber} (version ${data.version}) en base a los cambios solicitados.`,
    '',
    ...
    '',
    `Ver cotizacion actualizada: ${data.approvalLink}`,
  ]
    .filter(Boolean)
    .join('\n');
