...
import { createMockCliente, createMockTemplate } from '../../test/helpers/test-fixtures';
...
describe('RecurringInvoicesService', () => {
  ...
  describe('create', () => {
    ...
    it('should create template with nextRunDate calculated', async () => {
      ...
      expect(prisma.recurringInvoiceTemplate.create).toHaveBeenCalledWith(
    ...
    it('should stringify items to JSON', async () => {
      prisma.cliente.findFirst.mockResolvedValue(createMockCliente());
      prisma.recurringInvoiceTemplate.create.mockResolvedValue(createMockTemplate());

      await service.create('tenant-1', dto as Parameters<typeof service.create>[1]);
  ...
  describe('findAll', () => {
    it('should return paginated templates with cliente relation', async () => {
      const templates = [{ ...createMockTemplate(), cliente: createMockCliente(), _count: { history: 5 } }];
  ...
  describe('pause', () => {
    it('should set status to PAUSED from ACTIVE', async () => {
      prisma.recurringInvoiceTemplate.findFirst.mockResolvedValue(
        createMockTemplate({ status: 'ACTIVE' }),
      );
      prisma.recurringInvoiceTemplate.update.mockResolvedValue(
        createMockTemplate({ status: 'PAUSED' }),
      );
    ...
    it('should set status to PAUSED from SUSPENDED_ERROR', async () => {
      prisma.recurringInvoiceTemplate.findFirst.mockResolvedValue(
        createMockTemplate({ status: 'SUSPENDED_ERROR' }),
      );
      prisma.recurringInvoiceTemplate.update.mockResolvedValue(
        createMockTemplate({ status: 'PAUSED' }),
      );
    ...
    it('should throw BadRequestException from PAUSED state', async () => {
      ...
      await expect(service.pause('tenant-1', 'template-1')).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException when template not found', async () => {
      prisma.recurringInvoiceTemplate.findFirst.mockResolvedValue(null);

      await expect(service.pause('tenant-1', 'nonexistent')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('resume', () => {
    it('should set status to ACTIVE from PAUSED and reset failures', async () => {
      prisma.recurringInvoiceTemplate.findFirst.mockResolvedValue(
        createMockTemplate({ status: 'PAUSED', consecutiveFailures: 2 }),
      );
      prisma.recurringInvoiceTemplate.update.mockResolvedValue(
        createMockTemplate({ status: 'ACTIVE', consecutiveFailures: 0 }),
      );
    ...
    it('should throw BadRequestException from ACTIVE state', async () => {
      ...
      await expect(service.resume('tenant-1', 'template-1')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('cancel', () => {
    it('should set status to CANCELLED', async () => {
      ...
      prisma.recurringInvoiceTemplate.update.mockResolvedValue(
        createMockTemplate({ status: 'CANCELLED' }),
      );
    ...
    it('should throw BadRequestException if already CANCELLED', async () => {
      ...
      await expect(service.cancel('tenant-1', 'template-1')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getDueTemplates', () => {
    it('should find ACTIVE templates with nextRunDate <= now', async () => {
      const templates = [createMockTemplate({ nextRunDate: new Date('2020-01-01') })];
      prisma.recurringInvoiceTemplate.findMany.mockResolvedValue(templates);
  ...
  describe('getHistory', () => {
    ...
    it('should throw NotFoundException when template not found', async () => {
      ...
      await expect(
        service.getHistory('tenant-1', 'nonexistent', {}),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
