...
exports.NuevoClienteModal = NuevoClienteModal;
...
function NuevoClienteModal({ open, onClose, onCreated, tipoDte = '01', subtotal = 0, }) {
    ...
    return (<dialog_1.Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <dialog_1.DialogContent className="max-w-lg bg-background border-border">
        <dialog_1.DialogHeader>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-primary/20 flex items-center justify-center">
              <lucide_react_1.UserPlus className="w-5 h-5 text-primary"/>
            </div>
            <div>
              <dialog_1.DialogTitle className="text-lg">Nuevo Cliente</dialog_1.DialogTitle>
        ...
        <div className="space-y-4 py-4">
          ...
          {esConsumidorFinal && datosOpcionales && (<alert_1.Alert>
          ...
          {esConsumidorFinal && datosOpcionales && (<button_1.Button type="button" variant="outline" className="w-full border-primary/50 text-primary hover:bg-primary/10" onClick={handleFacturaRapida}>
          ...
          {esConsumidorFinal && subtotal < UMBRAL_DATOS_OBLIGATORIOS && (<div className="flex items-center space-x-2 p-3 rounded-lg bg-muted/50">
              <checkbox_1.Checkbox id="deducciones" checked={requiereDeducciones} onCheckedChange={(checked) => setRequiereDeducciones(checked === true)}/>
              <label_1.Label htmlFor="deducciones" className="text-sm cursor-pointer">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <select_1.Select value={formData.tipoDocumento} onValueChange={(value) => handleChange('tipoDocumento', value)}>
                <select_1.SelectTrigger>
                  <select_1.SelectValue />
                </select_1.SelectTrigger>
                <select_1.SelectContent>
                  {tiposDocDisponibles.map((tipo) => (<select_1.SelectItem key={tipo.value} value={tipo.value}>
                      {tipo.label}
                    </select_1.SelectItem>))}
                </select_1.SelectContent>
              </select_1.Select>
            ...
            <div className="space-y-2">
              ...
              <input_1.Input value={formData.numDocumento} onChange={(e) => handleChange('numDocumento', e.target.value)} placeholder={formData.tipoDocumento === '36' ? '0614-123456-101-2' : '01234567-8'}/>
              {errors.numDocumento && (<p className="text-xs text-destructive">{errors.numDocumento}</p>)}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <input_1.Input value={formData.nombre} onChange={(e) => handleChange('nombre', e.target.value)} placeholder={datosOpcionales ? 'Opcional - se usara "Clientes varios"' : 'Nombre completo o razon social'}/>
            {errors.nombre && <p className="text-xs text-destructive">{errors.nombre}</p>}
          </div>

          {/* NRC - SOLO para Credito Fiscal */}
          {esCreditoFiscal && (<div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-muted-foreground">
                  NRC *
                </label>
                <input_1.Input value={formData.nrc} onChange={(e) => handleChange('nrc', e.target.value)} placeholder="1234567"/>
              ...
              <div className="space-y-2">
                ...
                <input_1.Input value={formData.telefono} onChange={(e) => handleChange('telefono', e.target.value)} placeholder="2222-3333"/>
              </div>
            </div>)}

          {/* Telefono para Consumidor Final */}
          {esConsumidorFinal && (<div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Telefono</label>
              <input_1.Input value={formData.telefono} onChange={(e) => handleChange('telefono', e.target.value)} placeholder="2222-3333 (opcional)"/>
            </div>)}

          {/* Email */}
          <div className="space-y-2">
            <label className="text-sm font-medium text-muted-foreground">Correo Electronico</label>
            <input_1.Input type="email" value={formData.correo} onChange={(e) => handleChange('correo', e.target.value)} placeholder="correo@ejemplo.com (opcional)"/>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <select_1.Select value={formData.departamento} onValueChange={(value) => {
            ...
            if (muns && muns.length > 0) {
                ...
                <select_1.SelectContent className="max-h-[200px]">
                  {DEPARTAMENTOS.map((depto) => (<select_1.SelectItem key={depto.value} value={depto.value}>
                      {depto.label}
                    </select_1.SelectItem>))}
                </select_1.SelectContent>
              </select_1.Select>
            ...
            <div className="space-y-2">
              ...
              <select_1.Select value={formData.municipio} onValueChange={(value) => handleChange('municipio', value)}>
                <select_1.SelectTrigger>
                  <select_1.SelectValue />
                </select_1.SelectTrigger>
                <select_1.SelectContent className="max-h-[200px]">
                  {municipiosDisponibles.map((muni) => (<select_1.SelectItem key={muni.value} value={muni.value}>
                      {muni.label}
                    </select_1.SelectItem>))}
                </select_1.SelectContent>
              </select_1.Select>
          ...
          <div className="space-y-2">
            ...
            <input_1.Input value={formData.complemento} onChange={(e) => handleChange('complemento', e.target.value)} placeholder="Colonia, calle, numero... (opcional)"/>
          </div>
        </div>

        <dialog_1.DialogFooter className="gap-2">
          <button_1.Button variant="ghost" onClick={onClose} disabled={isSubmitting}>
            Cancelar
          </button_1.Button>
